...
Ehhez elsőként frissítsük az szenergy-autonom_packages repository-t!
...
git pull
```
Ezek után buildeljük az szenergy-autonom_local_planner nevű package-t!

``` r
cd ~/ros2_ws
``` 
``` r
colcon build --packages-select szenergy-autonom_local_planner
```
## Futtatás
...
``` 
``` r
ros2 launch szenergy-autonom_local_planner run_all.launch.py
```
...
- **szenergy-autonom_packages**
``` r
cd ~/ros2_ws/src
git clone https://github.com/sze-info/szenergy-autonom_packages.git
``` 
vagy
``` r
cd ~/ros2_ws/src/szenergy-autonom_packages
...
## Talajszűrő indítása
```r
ros2 launch szenergy-autonom_simple_perception filter_a.launch.py \
