...
!!! danger "Python megfelelője"
    A C++ kód python verziója szintén elérhető a [github.com/sze-info/szenergy-autonom_packages](https://github.com/sze-info/szenergy-autonom_packages/tree/main/szenergy-autonom_transforms_py) címen. Érdemes összehasonlítani a C++ és a python kódokat.

Frissítsük a legújabb verzióra az `szenergy-autonom_packages` repo-t. Ha frissül, vagy `Already up to date.` üzenetet kapunk, akkor nem kell klónoznunk. Ha a `cd ~/ros2_ws/src/szenergy-autonom_packages` parancs után a `~/ros2_ws/src/szenergy-autonom_packages: No such file or directory` üzenetetet kaptuk, akkor klónozzuk a repo-t.

``` bash
cd ~/ros2_ws/src/szenergy-autonom_packages
```
``` bash
git pull
```
...
!!! warning
    A következő két parancs csak nem létező `szenergy-autonom_packages` esetén kell:
...
```
``` bash
git clone https://github.com/sze-info/szenergy-autonom_packages
```
...
```
``` bash
colcon build --packages-select szenergy-autonom_transforms_cpp --symlink-install
```
...
```
``` bash
ros2 run szenergy-autonom_transforms_cpp pub_transforms
```
...
``` r
ros2 launch szenergy-autonom_transforms_cpp rviz1.launch.py
```

![transforms01](https://raw.githubusercontent.com/sze-info/szenergy-autonom_packages/main/szenergy-autonom_transforms_cpp/img/transforms01.gif)


Vizsgáljuk meg a `pub_transforms.cpp` fájlt. (Python esetén a `_py` package-ben a `transforms.py` fájlt.)

``` bash
cd ~/ros2_ws/src/szenergy-autonom_packages/szenergy-autonom_transforms_cpp
```
...
- node-ot, ami a `map`, `orbit1` és `orbit2` frame-ket publikálja (`ros2 run szenergy-autonom_transforms_cpp pub_transforms`)
...
- és az Rviz2-t indító launch-t is (`ros2 launch szenergy-autonom_transforms_cpp rviz1.launch.py`)
...
Megoldás: [elérhető az önálló feladatok között](https://sze-info.github.io/szenergy-autonom/onallo/ros2launchmarker/)
...
def generate_launch_description():
    ...
    pkg_name = 'szenergy-autonom_transforms_cpp'
    ...
    return LaunchDescription([
        ...
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource([
                FindPackageShare("szenergy-autonom_transforms_cpp"), '/launch/', 'rviz1.launch.py'])
        ),
    ])
```
...
[Python notebook quaternion](https://github.com/sze-info/szenergy-autonom/blob/main/docs/transzformaciok/gps_utm.ipynb){: .md-button .md-button-purple .mr-4 }


[gps_utm.ipynb](https://github.com/sze-info/szenergy-autonom/blob/main/docs/transzformaciok/quaternion.ipynb){: .md-button .md-button-purple .mr-4 } 
