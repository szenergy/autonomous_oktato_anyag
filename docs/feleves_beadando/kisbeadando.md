...
- [github.com/umiklos/ung_isl_szenergy-autonom_point_and_orientation](https://github.com/umiklos/ung_isl_szenergy-autonom_point_and_orientation): A package két node-ból áll. Az egyik node egy `geometry_msgs/Point` típust állít elő, a másik node pedig orientációval kiegészítve ebből egy `geometry_msgs/Pose` típusút hirdet. Megvalósítás `ROS 2 Humble` alatt.
- [github.com/umiklos/ung_isl_szenergy-autonom_data_generation_and_control](https://github.com/umiklos/ung_isl_szenergy-autonom_data_generation_and_control): A package két node-ból áll. Az `/sensor_data_generator` egy fiktív szenzorral szimulált adatokat generál, például távolságot és sebességet, ezeket két külön `sensor_msgs/Range` és `geometry_msgs/Twist` típusú topicban hirdeti. A másik node, a `/control_node` ezeket az adatokat figyeli és vezérlési döntéseket hoz a robot számára, amit kiír a terminalban. Megvalósítás `ROS 2 Humble` alatt.
