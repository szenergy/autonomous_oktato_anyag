...
Pár alapfogalom az [előző](https://sze-info.github.io/szenergy-autonom/bevezetes/ros2/) alkalomról: 
...
- **Package** (csomag): ROS programok (node-ok) gyűjteménye (pl. `turtlesim`, `szenergy-autonom_intro_cpp`, `szenergy-autonom_transforms_cpp`)
...
<figure markdown="span">
  ![](/szenergy-autonom/assets/images_common/turtlesim01.gif)
...
``` r
ls ~/ros2_ws/src | grep szenergy-autonom_
```
vagy 

``` c
cd ~ && test -d "ros2_ws/src/szenergy-autonom_packages" && echo Letezik || echo Nem letezik
```
...
```
``` bash
colcon build --packages-select szenergy-autonom_intro_cpp
```
...
``` r
ros2 run szenergy-autonom_intro_cpp cmd_gen_node
```

A következőképp mozog most a teknős:

<figure markdown="span">
  ![Image title](/szenergy-autonom/assets/images_common/turtlesim02.gif){ width="100%" }
  <figcaption>Turtle</figcaption>
</figure>


Forráskódja elérhető a [github.com/sze-info/szenergy-autonom_packages](https://github.com/sze-info/szenergy-autonom_packages/blob/main/szenergy-autonom_intro_cpp/src/cmd_gen_node.cpp) repon.A lényeg, hogy a `loop` függvény 5 Hz (200 ms) frekvencián fut le, és 
...
!!! important "Python megfelelője"
    A C++ kód python verziója szintén elérhető a [github.com/sze-info/szenergy-autonom_packages](https://github.com/sze-info/szenergy-autonom_packages/blob/main/szenergy-autonom_intro_py/szenergy-autonom_intro_py/cmd_gen_node.py) címen. Érdemes összehasonlítani a C++ és a python kódokat.

Nézzük meg az utolsó terminálban a Foxglove segítségével az élő adatokat (itt se felejtsük a `source`-t):

``` r
ros2 launch szenergy-autonom_intro_cpp foxglove_bridge.launch.py
```

Vizsgáljuk meg Foxglove Studio-val is WebSocketen keresztül (Open connection `ws://localhost:8765`):

![foxglove](/szenergy-autonom/assets/images_common/foxglove02.png)
...
``` r
ros2 launch szenergy-autonom_intro_cpp turtle.launch.py
```

Vizsgáljuk meg a package tartalmát röviden a `code ~/ros2_ws/src/szenergy-autonom_packages/szenergy-autonom_intro_cpp` parancs után.
