...
![](https://raw.githubusercontent.com/sze-info/szenergy-autonom_packages/main/szenergy-autonom_simple_perception/img/simple_filter01.gif)

Ha még nem tettük volna, klónozzuk az `szenergy-autonom_packages` repot és buildeljük az `szenergy-autonom_simple_perception` package-t.
...
``` r
MAKEFLAGS="-j4" colcon build --packages-select szenergy-autonom_simple_perception --cmake-args -DCMAKE_BUILD_TYPE=Release
```
...
!!! tip
    ...
    További olvasnivaló a [Troubleshooting](/szenergy-autonom/onallo/troubleshoot/) fejezetben.
...
``` r
ros2 run szenergy-autonom_simple_perception lidar_filter_simple
```
...
![Alt text](/szenergy-autonom/assets/images_common/rqt_graph02.svg)
...
``` r
code ~/ros2_ws/src/szenergy-autonom_packages/szenergy-autonom_simple_perception
```
...
![compare_vs_code01](/szenergy-autonom/assets/images_common/compare_vs_code01.png)
...
``` r
ros2 run szenergy-autonom_simple_perception lidar_filter_simple_param
```
...
``` r
ros2 launch szenergy-autonom_simple_perception run_rviz1.launch.py
```
...
``` r
ros2 launch szenergy-autonom_simple_perception run_all.launch.py
```

Nagyjából így fog kinézni az `rqt_reconfigure` meg az `rviz2`:

![](/szenergy-autonom/assets/images_common/rqt_rviz01.png)
...
``` r
ros2 launch szenergy-autonom_simple_perception run_filter_and_rviz.launch.py
```
...
!!! Tip
    Szebb megoldás a `IncludeLaunchDescription` - `PythonLaunchDescriptionSource` használata. Példa lehet a [ros2launchmarker.md](/szenergy-autonom/onallo/ros2launchmarker/#launch-fajl-letrehozasa) fejezetben található kódrészlet.
